...
# Model Evaluation with Hold-out
...
## CV ignores the features come from fit() method.
...
cart_bestGS =GridSearchCV(cart_model, cart_params, cv = 5, n_jobs=-1, verbose=True).fit(X, y)

cart_bestGS.best_params_

cart_bestGS.best_score_ ##accuracy

random = X.sample(1, random_state=45)

cart_bestGS.predict(random)


# Final Model

cart_final = DecisionTreeClassifier(**cart_bestGS.best_params_, random_state=17).fit(X, y)
cart_final.get_params()

cart_final = cart_model.set_params(**cart_bestGS.best_params_).fit(X, y)
...
def plot_importance(model, features, num=len(X), save = False):
    feat_imp = pd.DataFrame({'Value': model.feature_importances_, 'Feature': features.columns})
    plt.figure(figsize=(10, 10))
    sns.set(font_scale = 1)
    sns.barplot(x='Value', y='Feature', data=feat_imp.sort_values(by="Value", ascending=False)[0:num])
    plt.title('Features')
    plt.tight_layout()
    plt.show()
    if save:
...
plt.plot(range(1,11), mean_test_score, label = "Validating Score", color = 'g')
